"""API for bioRxiv and medRXiv. Extracted and adapted from paperscraper."""
        ...
        try:
            ...
            if start_date:
                try:
                    ...
                    logger.warning(f"Invalid start_date format: {start_date}, using launch_date")
                    start_date = self.launch_date
            else:
                start_date = self.launch_date
            ...
            while do_loop:
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        if not json_response or "messages" not in json_response:
                        ...
                        do_loop = False # Stop loop on unexpected error to prevent infinite loop
                        break
        except Exception as exc:
            logger.exception(f"Failed getting papers: {exc}")
